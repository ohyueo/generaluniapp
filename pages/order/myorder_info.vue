	...
	<view v-if="list && list.id">
		...
		<view class="" style="padding: 0rpx 20rpx;">
			<view class="u-flex-col u-bg-white" style="padding: 0 10px;">
				...
				<view class="u-flex u-flex-wrap" style="line-height: 60rpx;color: #333333;font-size: 24rpx;" v-if="list && list.riqi">
		...
		<view class="myorderboot u-bg-white u-flex" :class="isqushow==1?'u-row-between':'u-row-center'" v-if="list.status==2">
			...
			<view v-if="hexiaotype==0" @click="open()" class="u-flex u-row-center u-col-center zhuti_bg u-color-white" :style="isqushow==1?'width: 300rpx;':'width: 600rpx;'" style="height: 70rpx;border-radius: 35rpx;margin-right: 20rpx;">核销二维码</view>
			<view v-if="hexiaotype==1" @click="saoma()" class="u-flex u-row-center u-col-center zhuti_bg u-color-white" :style="isqushow==1?'width: 300rpx;':'width: 600rpx;'" style="height: 70rpx;border-radius: 35rpx;margin-right: 20rpx;">
				<text class="icon" style="font-size: 38rpx;">&#xe62d;</text>
				<text class="u-margin-left-10">扫码签到</text>
			</view>
		</view>
		<!--取消订单-->
		...
		methods: {
			...
			getData(){
				...
				this.$api.myorderinfo({'token':this.token,id:this.id,type:type}).then((res)=>{
					...
					_this.hexiaotype=res.data.data.hexiaotype;
					_this.isqushow=res.data.data.isqushow;
					if(res.data.data.isding==1){
						_this.dingwei();
					}
