			...
			<view class="class_right">
				...
				<view class="class_rightli u-flex-col">
					...
					<view class="u-bg-white" style="width: 550rpx;border-bottom-left-radius: 20rpx;border-bottom-right-radius: 20rpx;padding: 20rpx 0;">
						...
						<view class="u-flex u-margin-top-20 u-flex-wrap" style="padding: 0 10rpx;">
							<block v-for="(item,index) in list" :key="index">
								<view class="timeli u-flex-col u-row-center u-col-center u-margin-bottom-10" v-for="(itemx,index2) in item.list" :key="index2" @click="tiaourl(itemx.status,itemx.time,item.id,itemx.title)"  :class="itemx.status==1 && time!=itemx.time?'u-color-balck3':itemx.status==1 && time==itemx.time?'u-color-white xuan zhuti_bg':'u-color-balck9'">
									<text class="u-font-26">{{itemx.time}}</text>
									<text class="u-font-24 u-margin-top-10">{{itemx.title}}</text>
								</view>
							</block>
...
<script>
	export default {
		...
		methods: {
			getData(){
				var _this=this;
				this.$api.getydate({id:this.id,token:this.token,cid:this.cid}).then((res)=>{
			...
			getytime(){
				var _this=this;
				this.$api.getytime({id:this.id,riqi:this.riqi,token:this.token,cid:this.cid}).then((res)=>{
			...
			tiaox(){
				if(this.time){
					if(this.opst==3){
						var url="order?id="+this.id+'&riqi='+this.riqi+"&time="+this.time+'&wek='+this.wek+'&timeid='+this.timeid+'&cid='+this.cid;
						this.tiao(url);
					}else if(this.opst==4){
						var url="seat?id="+this.id+'&riqi='+this.riqi+"&time="+this.time+'&wek='+this.wek+'&timeid='+this.timeid+'&cid='+this.cid;
						this.tiao(url);
					}
				}else{
					uni.showToast({
