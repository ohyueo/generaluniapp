<template>
	<view class="content">
		...
		<view class="u-flex u-col-center u-row-between u-padding-20" v-if="lunbo && lunbo.length>0">
			<view class="imglist">
		...
		<view class="notilist u-border-ra20" v-if="noti && noti.length>0">
			<view class="u-flex u-row-between u-row-center u-padding-20 u-col-center">
				<text class="icon zhuti_color u-font-32">&#xe609;</text>
				<view class="noticent">
					<swiper  :vertical="true" :autoplay="true" style="height: 40rpx;line-height: 40rpx;">
						<swiper-item class="u-font-dan-sheng" @click="tiao('../texter/noti_info?id='+item.id)" v-for="(item,index) in noti" :key="index">
							<text class="u-font-28 u-color-balck3 u-font-dan-sheng u-margin-left-10" style="width: 620rpx;">{{item.title}}</text>
						</swiper-item>
					</swiper>
				</view>
		...
		<!--小按钮-->
		<view class="u-flex u-flex-wrap u-row-between" style="width:710rpx;background-color: #F6F6F6;" v-if="kuai && kuai.length>0">
		...
		<view class="u-flex-col gxcent u-margin-bottom-20 u-border-ra20 u-bg-white u-margin-top-20" v-if="list && list.length>0">
			...
			<view v-for="(item,index) in list" :key="index" class="u-padding-20 u-flex border-bootom-1" @click="tiaoinfo(item.id,item.opst)">
				...
				<view class="u-flex-col u-margin-left-20 clix u-row-between">
					...
					<view class="u-flex u-row-between" v-if="item.recommended">
						<text class="u-font-24 u-color-balck9">{{item.recommended}}</text>
						<view class="lxbtn">
		...
		<!--新闻动态-->
		<view class="u-flex-col gxcent u-margin-bottom-20 u-border-ra20 u-bg-white" v-if="textlist && textlist.length>0">
			<view class="u-padding-20 u-flex border-bootom-1">
				<view class="xiaoli zhuti_bg"></view>
				<span class="u-font-28 u-color-balck3 u-margin-left-10">{{newtit}}</span>
			</view>
			<view class="u-flex u-padding-20 u-border-bottom-1" v-for="(item,index) in textlist" :key="index" @click="tiao('../texter/texter_info?id='+item.id)">
				<view class="u-flex u-row-between u-flex-1">
					<view class="u-flex-col" style="width: 520rpx;">
		...
		<!-- <image src="../../static/logo.png" @load="wxParseImgLoad"></image> -->
		...
		methods: {
			...
			getData(){
				...
				this.$api.indeximg().then((res)=>{
					_this.newtit=res.data.data.newtit;
